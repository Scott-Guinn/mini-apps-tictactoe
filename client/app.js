...
var rotateBoard = () => {
  var rotatedBoard = [[board[2][0], board[1][0], board[0][0]], [board[2][1], board[1][1], board[0][1]], [board[2][2], board[1][2], board[0][2]]];
  board = rotatedBoard;
}
...
var gravity = () => {
  ...
  for (var col = 0; col < 3; col++) {
    // declare currentRow, currentCol vars
    var row = 2;
    // while row is less than 2...
    while (row > 0) {
      // if square is empty (=== null)
      if (board[row][col] === null) {
        // iterate through the squares above (up to two possible squares)
        for (var rowAbove = row-1; rowAbove >= 0; rowAbove--) {
          // if square above contains a value (!== null)
          if (board[rowAbove][col] !== null) {
            // set currentSquare to that value
            board[row][col] = board[rowAbove][col];
            // erase that value
            board[rowAbove][col] = null;
          }
        }
      }
      // decrement search row
      row--;
    }
  }
}
